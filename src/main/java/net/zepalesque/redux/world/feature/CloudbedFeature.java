...
public class CloudbedFeature extends Feature<CloudbedFeature.Config> {
    ...
    public boolean place(FeaturePlaceContext<Config> context) {
        ...
        // Extra stuff, if should override compat then always generate
        if (Redux.ancientAetherCompat() && !config.overrideCompat().orElse(false))
            return false;
    ...
    public record Config(BlockStateProvider block, BlockPredicate predicate, int yLevel, DensityFunction cloudNoise, double cloudRadius, DensityFunction yOffset, double maxYOffset, Optional<Boolean> overrideCompat) implements FeatureConfiguration {
        public static final Codec<Config> CODEC = RecordCodecBuilder.create(
                (builder) -> builder.group(
                        ...
                        Codec.BOOL.optionalFieldOf("override_compat_disable").forGetter(Config::overrideCompat)
